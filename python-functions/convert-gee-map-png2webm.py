...
bucket_name = "D:/data/eo-bathymetry"
folder = "sdb-rgb-test4"
temp_dir = "temp"
video_folder = folder+"-video"

steps = glob(folder + '/*')
      
zoom_levels = range(4, 14)
...
for i, step in enumerate(steps):
    ...
    for zoom in zoom_levels:
        zoom_dir =  r'{0}/{2}/{3}'.format(bucket_name, folder, step, zoom)
...
for zoom in zoom_levels:
    ...
    for tile_x in tile_x_list:
        ...
        for tile_y in tile_y_list:
            ...
            if not os.path.exists(new_path):
                os.makedirs(new_path)
            cmd = 'ffmpeg -framerate 10 -i "{0}/{1}/%03d.png" -c:v libvpx -an -qmin 0 -qmax 30 -crf 5 -auto-alt-ref 0 {2}/{1}.webm -y'.format(tile_x_dir, tile_y, new_path)
            run(cmd)
